...
## 완료 (질문 추가 버튼 강조)
- [x] 툴바 "질문 추가"를 별도 `ToolbarItem(.primaryAction)` + `.borderedProminent`(강조색 파란 채움)
      (창 제목을 숨긴 레이아웃이라 오른쪽 끝으로 가지 않고 "보기" 옆에 그대로 붙음 — 실행 화면으로 확인)

## 완료 (지원·개인정보 처리방침 페이지 갱신)
- [x] 기존 `docs/support.html`·`docs/privacy.html`(7/25, 라이브) 을 같은 URL 그대로 현재 앱에 맞게 갱신 — 이름 '질문 노트 (LectureQ)'
- [x] 지원: FAQ를 시작하기/정리하기/설정·데이터로 묶고 현행 기능 반영
      (질문 추가 버튼·시점, 강의 메뉴, ? 표시 의미, 타임블록 설정·자동 블록, 꼬리질문, 해결됨/배운 점,
       보기 메뉴 ⌘1~4, AI 프롬프트 복사, 마크다운 가져오기 형식, 글씨 크기 ⌘+/-/0, 삭제, 백업·초기화)
      — 없어진 'Q 버튼'·'− aA +' 안내 제거
- [x] 개인정보: 파일 가져오기(사용자 선택 파일 읽기 전용), 클립보드·외부 AI 서비스, 가이드 링크(브라우저) 항목 추가,
